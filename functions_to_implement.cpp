...
std::vector<bool> OddMask(std::vector<int> nums) {
    ...
    for (int num : nums) {
        result.push_back(num % 2 != 0); // true if odd, false if even
    }
    return result;
}
...
std::vector<bool> GreaterMask(std::vector<int> nums, int greater_than) {
    std::vector<bool> result;
    for (double num : nums) {
        result.push_back(num > greater_than);
    }
    return result;
}
...
//     std::cout << "OddMask result: ";
//     for (bool isOdd : result) {
//         std::cout << (isOdd ? "true " : "false ");
//     }
//     std::cout << std::endl;
